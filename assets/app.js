...
if (form) {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    if (!name || !phone || !memo) {
      setMessage("이름, 휴대폰 번호, 문의 내용은 필수입니다.", "error");
      return;
    }
    ...
    try {
      const res = await fetch(`${API_BASE}/api/apply`, {
        ...
        body: JSON.stringify({ name, phone, email, product: "general_inquiry", memo }),
      ...
      setMessage("문의가 접수되었습니다. 빠르게 연락드리겠습니다.", "success");
      form.reset();
    } catch (err) {
      clearTimeout(timeoutId);
